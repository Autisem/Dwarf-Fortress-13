...
/obj/machinery/icecream_vat/Initialize()
	...
	for(var/reagent in icecream_vat_reagents)
		reagents.add_reagent(reagent, icecream_vat_reagents[reagent], reagtemp = 273.15)
	reagents.chem_temp = 273.15 //So ice doesn't melt
		...
		if(!I.ice_creamed)
			if(product_types[dispense_flavour] > 0)
				...
				if(I.reagents.total_volume < 10)
					I.reagents.add_reagent(/datum/reagent/consumable/sugar, 10 - I.reagents.total_volume, reagtemp = 273.15)
				updateDialog()
			else
...
/obj/item/food/icecream/proc/set_cone_type(cone_name)
	...
	switch (cone_type)
		if ("waffle")
			reagents.add_reagent(/datum/reagent/consumable/nutriment, 1, reagtemp = 273.15)
		if ("chocolate")
			reagents.add_reagent(/datum/reagent/consumable/coco, 1, reagtemp = 273.15) // chocolate ain't as nutritious kids
