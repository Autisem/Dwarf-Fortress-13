...
/obj/item/clothing/head/mob_holder/on_found(mob/finder)
	if(held_mob?.will_escape_storage())
		to_chat(finder, span_warning("[capitalize(held_mob.name)] pops out!"))
		finder.visible_message(span_warning("[capitalize(held_mob.name)] pops out of the container [finder] is opening!") , ignored_mobs = finder)
		release(TRUE, FALSE)
		return
