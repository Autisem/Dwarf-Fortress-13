...
/datum/reagent/medicine/c2/hercuri/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	M.adjust_bodytemperature(rand(-25,-5) * 1.5 * REM * delta_time, 50)
	if(ishuman(M))
		var/mob/living/carbon/human/humi = M
		humi.adjust_coretemperature(rand(-25,-5) * 1.5 * REM * delta_time, 50)
...
/datum/reagent/medicine/c2/hercuri/expose_mob(mob/living/carbon/exposed_mob, methods=VAPOR, reac_volume)
	...
	exposed_mob.adjust_bodytemperature(-reac_volume * 1.5, 50)
...
/datum/reagent/medicine/c2/hercuri/overdose_process(mob/living/carbon/M, delta_time, times_fired)
	M.adjust_bodytemperature(-10 * 1.5 * REM * delta_time, 50) //chilly chilly
	if(ishuman(M))
		var/mob/living/carbon/human/humi = M
		humi.adjust_coretemperature(-10 * 1.5 * REM * delta_time, 50)
	..()
...
/datum/reagent/medicine/c2/seiver/on_mob_life(mob/living/carbon/human/M, delta_time, times_fired)
	...
	//and you're cold
	var/radcalc = round((273.15-chemtemp) / 6, 0.1) * REM * delta_time //max ~45 rad loss unless you've hit below 0K. if so, wow.
	if(radcalc > 0)
